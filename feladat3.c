...
void titkosit(char bemenet[], char kimenet[]) {
	...
	for(int i = 0; bemenet[behossz] != '\0'; ++i) {
		++behossz;
	}
    int sizeki = 0;

    for (int i = 0; i < behossz; ++i) {
        int szohossza = 0;
        for(szohossza = 0; bemenet[i] != ' ' && bemenet[i] != '\0'; ++szohossza) {
            ++i;
        }
        int szohosszint = szohossza + '0';
        if (szohossza != 0) {
            kimenet[sizeki] = szohosszint;
            ++sizeki;
        }
    }

    // szavak vissza fél kiírása

 	int aktki = sizeki;
 	int hozzaad = 0;
	for (int j = 0; j < aktki; ++j) {
        int kimenetint = kimenet[j] - '0';
		int bemenetaktualis = kimenetint + hozzaad - 1;
		for (int k = 0; k < kimenetint; ++k) {
			kimenet[sizeki] = bemenet[bemenetaktualis];
			bemenetaktualis--;
			sizeki++;
		}
		hozzaad = 1 + hozzaad + kimenetint;
	}
	kimenet[sizeki]='\0';
}
/***********************************************************************
